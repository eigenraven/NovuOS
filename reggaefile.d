...
enum DFLAGS_FREESTANDING = `-vgc -g -mtriple=x86_64-unknown-linux-elf -disable-red-zone -Iuefi-d/source -Isource/uefiloader/dsrc -nogc -defaultlib= -debuglib= -code-model=large -mattr=+sse,+sse2,-sse3 `;
...
{
	return objectFile(SourceFile(`source/kernel/novuos/`~mod),Flags(DFLAGS_FREESTANDING), KERNEL_IMPORTS, KERNEL_SIMPORTS);
}
...
enum kernel_objs = kObjs([`../object2.d`,`kmain.d`, `bootdata.d`, `basictypes.d`, `memory/pager.d`, `gfx/framebuffer.d`, `gfx/fbcon.d`, `formats/elf.d`, `cpu/descriptors.d`]);
